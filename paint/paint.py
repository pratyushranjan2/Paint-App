...
class PaintApp:
    drawing_tool = "line"
    left_but = "up"     # to record left mouse clicks
    colour = "black"
    background_colour = "white"
    width = 1
    x_pos,y_pos = None,None     # assisting in making shapes
    x1_line_pt,y1_line_pt,x2_line_pt,y2_line_pt = None,None,None,None
    r,diag,width,height = None,None,None,None
    theta = math.pi/4
    start_x,start_y=None,None
    clicktool=0  # to record the click of some tool in toolbar. necessary for adjusting default thickness of some tools.

    def save(self):   # function to save the canvas content
        self.drawing_area.update()
        file_name = asksaveasfilename()
        self.drawing_area.postscript(file = file_name + ".ps", colormode = 'color')

    def change_tool(self,tool):  # a function to change the drawing tool whenever clicked
        self.drawing_tool = tool
        self.clicktool=1         # records the click of a tool in toolbar


    def change_colour(self,colour):  # function to change the colour of drawing 
        self.colour = colour

    def left_but_down(self,event = None):  # a function to record left mouse clicks and perform some necessary actions
        self.left_but = "down"
        self.x1_line_pt = event.x  # recording the coordinates of the click
        self.y1_line_pt = event.y
        self.start_x=event.x
        self.start_y=event.y

    def left_but_up(self,event = None): # when the mouse click is released this function records it and performs some necessary actions
        self.left_but = "up"
        self.start_x=None
        self.start_y=None
        ...
        self.y2_line_pt = event.y  # records the coordinates of the point where the mouse key is released
        
        if self.drawing_tool == "line":  # performs the drawing functions accordingly
    ...
    def motion(self,event = None):   # some drawings which need continuation like pencil,eraser etc come under this function. This function is called in every event.
        if self.drawing_tool == "pencil":
            if self.clicktool==1:
                self.slider.set(0)  # setting of the default thickness of the pencil. The user may change it as he/she wants by adjusting the slider at the top. 
                self.clicktool=0
    ...
    def pencil_draw(self,event = None):  # function for drawing by a pencil
    ...
    def eraser(self,event = None):       # function to erase the drawing contents.
        if self.left_but == "down":
            if self.x_pos is not None and self.y_pos is not None:
    ...
    def square_tunnel(self,event=None):     # function to make a rectangular tunnel kind of shape.
    ...
    def circular_tunnel(self,event=None):   # function to make a circular disc/tunnel kind of shape
    ...
    def line_draw(self,event = None):      # functions for drawing various shapes
    ...
    def clear_canvas(self):     # function to clear the entire canvas
    ...
    def __init__(self,root,leftframe,topframe,width,height,pencil_img,rectangle_img,oval_img,square_img,eraser_img,paint_brush_img,i,a):
        ...
        self.width=width  # adjusting the width and the height of the canvas
        self.a = a
        self.height=height
        ...
        self.drawing_area.bind("<Motion>",self.motion)  # in every event all the records of motion is recorded and sent to the motion function 
        self.drawing_area.bind("<ButtonPress-1>",self.left_but_down)  # if LMB is clicked left_but_down function is called
        self.drawing_area.bind("<ButtonRelease-1>",self.left_but_up)  # if LMB is released left_but_up function is called
...
width = 1700
height = 1000
root.minsize(width,height)        # adjusting the width and height of the tinter window
# creation of the leftframe and the top frame
